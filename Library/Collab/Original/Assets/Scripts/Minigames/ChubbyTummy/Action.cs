...
public class Action : NetworkBehaviour
{
    ...
    void OnTriggerEnter2D(Collider2D col)
    {
        ...
        else if (col.gameObject.tag.Equals("Candy"))
        {
            ...
            myPlayerInfo.UpdateLocalScore(myPlayerInfo.localCharacterScore);
            CmdUpdateLocalScore(myPlayerInfo.localCharacterScore);
        }
    }
    ...
    {
        //QUI ARRIVA LO SCORE MA myPlayerInfo è null

        print("SCORE ARRIVATO DAL CLIENT netID " + netId + "è " + score);

        if (myPlayerInfo == null)
            return;

        myPlayerInfo.UpdateLocalScore(score);

        print("[3] Command: Actual score is " + myPlayerInfo.localCharacterScore + " and received score is: " + score + ", then calling RPC from netID: " + netId.Value.ToString());

        myPlayerInfo.localCharacterScore = score;
    ...
    {
        if (myPlayerInfo == null || myPlayerInfo.localCharacterScore == score)
            return;
