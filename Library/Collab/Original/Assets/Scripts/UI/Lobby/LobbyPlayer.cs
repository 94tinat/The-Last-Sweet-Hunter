...
public class LobbyPlayer : NetworkLobbyPlayer {
    ...
    * *******************************************/
    ...
    {
        if (localCharacterScore == score)
            return;

        print("[1] HOOK = UpdateLocalScore is trying to update localCharacterScore from " + localCharacterScore + " to " + score);

        localCharacterScore = score;

        print("[2] Client: New score is: " + score + ", then calling Command from netID: " + netId.Value.ToString());

        CmdUpdateLocalScore(score);
    }

    [Command]
    public void CmdUpdateLocalScore(int score)
    {
        print("[3] Command: Actual score is " + localCharacterScore + " and received score is: " + score + ", then calling RPC from netID: " + netId.Value.ToString());
    ...
    [ClientRpc]
    public void RpcUpdateLocalScore(int score)
    {
        if (localCharacterScore == score)
            return;

        print("[4] RPC: Received score is: " + score + ", then updating score " + score + " from netID: " + netId.Value.ToString());
        localCharacterScore = score;
    }


    //[Client]
    // public void OnCharacterScoreChange(int value)
    // {
    //     localCharacterScore = value;
    //     // invoke the change on the Server as you already named the function
    //     CmdUpdateScores(value, this.globalCharacterScore);
    //     print("Updating new score " + value +" for index and netID: " + characterIndex + " " + netId.Value.ToString());
    // }

    // // invoked by clients but executed on the server only
    // [Command]
    // void CmdUpdateScores(int local, int global)
    // {
    //     //Debug.Log("°°°°°°°°°°°°° Sono nella cmd");
    //     OnCharacterScoreChange(local);
    //     // make the change local on the server
    //     localCharacterScore = local;
    //     globalCharacterScore = global;

    //     // forward the change also to all clients
    //     RpcUpdateScores(local, global);
    // }

    // // invoked by the server only but executed on ALL clients
    // [ClientRpc]
    // void RpcUpdateScores(int local, int global)
    // {

    //     //Debug.Log("°°°°°°°°°°°°° Sono nella rpc");
    //     OnCharacterScoreChange(local);
